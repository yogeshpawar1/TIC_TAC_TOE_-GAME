...
int main(){
    ...
    while(listsof_element.size()!=0){
        if(flag==true){
             draw(arr);
             again:
             cout<<"enter first player any number betwee 1 to 9 and present space : ";
             int user_number;
             cin>>user_number;
             ...
             draw(arr);
             again2:
             cout<<"enter second player any number betwee 1 to 9 and present space : ";
             int user_number2;
             cin>>user_number2;
             auto it_f_ound=find(listsof_element.begin(),listsof_element.end(),user_number2);
             if(it_f_ound==listsof_element.end()) goto again2;
             else{
                 set_variable(arr,user_number2,'X');
                listsof_element=element_check(arr);
                flag=true;
             }
       ...
       win=win_check(arr);
       draw(arr);
    if(win=='W') cout<<"player first won the match: "<<endl;
    else if(win=='L') cout<<"player second won the match : "<<endl;
